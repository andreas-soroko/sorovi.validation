...
namespace sorovi.Validation
{
    public static class ValidationIfNull
    {
        ...
        {
            ExceptionFactory.ThrowIf<TException>(arg.Value is null, type, message ?? $"Expected '{arg.MemberName}' not to be <null>", arg.MemberName, arg.Value);
            return ref arg;
        }
        ...
        public static ref readonly ArgumentInfo<T> IfNotNull<T, TException>(this in ArgumentInfo<T> arg, in string type = ValidationTypes.ValueNull, in string message = null)
            where TException : Exception
        {
            ExceptionFactory.ThrowIf<TException>(!(arg.Value is null), type, message ?? $"Expected '{arg.MemberName}' to be <null>", arg.MemberName, arg.Value);
            return ref arg;
        }
    }
}