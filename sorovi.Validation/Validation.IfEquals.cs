...
namespace sorovi.Validation
{
    public static class ValidationIfEquals
    {
        ...
        {
            ExceptionFactory.ThrowIf<TException>(EqualityComparer<T>.Default.Equals(arg.Value, compareValue), type, message ?? $"Expected '{arg.MemberName}' not to be <null>", arg.MemberName, arg.Value);
            return ref arg;
        }
        ...
        public static ref readonly ArgumentInfo<T> IfNotEqualsTo<T, TException>(this in ArgumentInfo<T> arg, in T compareValue, in string type = ValidationTypes.ValueNull, in string message = null)
            where TException : Exception
        {
            ExceptionFactory.ThrowIf<TException>(!EqualityComparer<T>.Default.Equals(arg.Value, compareValue), type, message ?? $"Expected '{arg.MemberName}' to be <null>", arg.MemberName, arg.Value);
            return ref arg;
        }
    }
}